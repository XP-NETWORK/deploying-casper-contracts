...
import { CLAccountHash, CLAnyType, CLByteArray, CLByteArrayBytesParser, CLList, CLListBytesParser, CLU256, CLU256BytesParser, CLU512, CLU512BytesParser, CLU8BytesParser, CLValueBuilder, CLValueParsers, CasperClient, Keys, RuntimeArgs } from "casper-js-sdk";
...
const RoyaltySerialzer = (royalties: Royalties[]) => {
    ...
    const byteArray = new CLByteArrayBytesParser()
    const listParser = new CLListBytesParser()
    ...
    for (const royalty of royalties) {
        ...
        if (royaltyType === "FlatRoyalty") {
            ...
            const bytes = CLValueBuilder.byteArray(Buffer.concat([u8Parser.toBytes(u8).unwrap(), clu512ByteParser.toBytes(clu512).unwrap()]))
            ...
            const bytes = CLValueBuilder.byteArray(Buffer.concat([u8Parser.toBytes(u8).unwrap(), clu256Parser.toBytes(clu256).unwrap()]))
            clListBuilder.push(bytes)
        }
    }
...
(async () => {
    const whitelistedMarketPlacesStr = await new Promise<string>((res) =>
        read.question("Enter White Listed Markey Place's ContractPackageHash (seprated by ,): ", res)
    );
    ...
    const deployed = await contract.install(readFileSync("./src/royalty.wasm"), RuntimeArgs.fromMap({
        whitelisted_marketplaces: new CLList(whitelisted_marketplaces),
        royalty_structure: RoyaltySerialzer(royalties),
        manager: managerAccountHash,
    }), "300000000000", key.publicKey, NETWORK, [key]).send(NODE);

    console.log(`Deploy Hash: `, deployed);

    const result = await getDeploy(NODE, deployed);

    console.log(`Deployed Result:`, JSON.stringify(result, null, 4));
